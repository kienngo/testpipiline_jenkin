...
{
    public class GroupPatientJnlAccess : IGroupPatientJnlAccess
    {
        private IDBAccess _access;

        private readonly string _groupPatientJnl_tb = "GRP_GROUPPATIENTJNL";

        /// <summary>
        /// コンストラクタ
        /// </summary>
        public GroupPatientJnlAccess(IDBAccess access)
        {
            _access = access;
        }

        public int Create(GroupPatientJnlEntity content)
        {
            var parameters = new Parameters();

            var sb = new StringBuilder();
            sb.AppendLine("INSERT INTO");
            sb.AppendLine($" {_groupPatientJnl_tb}");
            sb.AppendLine("(");
            ...
            sb.AppendLine(", GROUPPATIENT_CODE");
            sb.AppendLine(", GROUPMANAGEMENT_ID");
            sb.AppendLine(", PATIENT_ID");
            ...
            sb.Append(SQLFunction.Char(content.GROUPPATIENT_CODE));
            sb.Append(',');

            sb.Append(SQLFunction.Char(content.GROUPMANAGEMENT_ID));
            sb.Append(',');

            sb.Append(SQLFunction.Char(content.PATIENT_ID));
            sb.Append(',');
