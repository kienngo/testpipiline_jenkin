...
{
    public class CategorySelectedJnlAccess : ICategorySelectedJnlAccess
    {
        private IDBAccess _access;

        private readonly string _groupCategorySelectedJnl_tb = "GRP_CATEGORYSELECTEDJNL";


        /// <summary>
        /// コンストラクタ
        /// </summary>
        public CategorySelectedJnlAccess(IDBAccess access)
        {
            _access = access;
        }

        public int Create(CategorySelectedJnlEntity content)
        {
            var parameters = new Parameters();

            var sb = new StringBuilder();
            sb.AppendLine("INSERT INTO");
            sb.AppendLine($" {_groupCategorySelectedJnl_tb}");
            sb.AppendLine("(");
            sb.AppendLine("  HISTORY_SEQ");
            sb.AppendLine(", LOCKVERSION");
            sb.AppendLine(", OPERATION");
            sb.AppendLine(", REC_ID");
            sb.AppendLine(", CATEGORYSELECTED_CODE");
            ...
            sb.Append(SQLFunction.Char(content.CATEGORYSELECTED_CODE));
            sb.Append(',');

            sb.Append(SQLFunction.Char(content.GROUPCATEGORY_ID));
            sb.Append(',');

            sb.Append(SQLFunction.Char(content.CATEGORY_ID));
            sb.Append(',');
