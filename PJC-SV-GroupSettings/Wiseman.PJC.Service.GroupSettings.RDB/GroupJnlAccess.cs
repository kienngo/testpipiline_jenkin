...
{
    public class GroupJnlAccess : IGroupJnlAccess
    {
        private IDBAccess _access;

        private readonly string _groupJnl_tb = "GRP_GROUPJNL";


        /// <summary>
        /// コンストラクタ
        /// </summary>
        public GroupJnlAccess(IDBAccess access)
        {
            _access = access;
        }

        public int Create(GroupJnlEntity content)
        {
            var parameters = new Parameters();

            var sb = new StringBuilder();
            sb.AppendLine("INSERT INTO");
            sb.AppendLine($" {_groupJnl_tb}");
            sb.AppendLine("(");
            ...
            sb.AppendLine(", GROUP_KANA");
            sb.AppendLine(", GROUP_RYAKUSHO");
            sb.AppendLine(", VALID_FLAG");
            ...
            sb.Append(SQLFunction.Char(content.GROUP_CODE));
            sb.Append(',');

            sb.Append(SQLFunction.Char(content.GROUPCATEGORY_ID));
            sb.Append(',');

            sb.Append(SQLFunction.Char(content.GROUP_NAME));
            sb.Append(',');

            sb.Append(SQLFunction.Char(content.GROUP_KANA));
            sb.Append(',');

            sb.Append(SQLFunction.Char(content.GROUP_RYAKUSHO));
            sb.Append(',');

            sb.Append(SQLFunction.Char(content.VALID_FLAG));
            sb.Append(',');

            sb.Append(SQLFunction.Char(content.REMARKS));
            sb.Append(',');
