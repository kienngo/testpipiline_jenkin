...
{
    public class GroupStaffJnlAccess : IGroupStaffJnlAccess
    {
        private IDBAccess _access;

        private readonly string _groupStaffJnl_tb = "GRP_GROUPSTAFFJNL";

        /// <summary>
        /// コンストラクタ
        /// </summary>
        public GroupStaffJnlAccess(IDBAccess access)
        {
            _access = access;
        }

        public int Create(GroupStaffJnlEntity content)
        {
            var parameters = new Parameters();

            var sb = new StringBuilder();
            sb.AppendLine("INSERT INTO");
            sb.AppendLine($" {_groupStaffJnl_tb}");
            sb.AppendLine("(");
            ...
            sb.Append(SQLFunction.Char(content.GROUPSTAFF_CODE));
            sb.Append(',');

            sb.Append(SQLFunction.Char(content.GROUPMANAGEMENT_ID));
            sb.Append(',');

            sb.Append(SQLFunction.Char(content.STAFF_ID));
            sb.Append(',');
