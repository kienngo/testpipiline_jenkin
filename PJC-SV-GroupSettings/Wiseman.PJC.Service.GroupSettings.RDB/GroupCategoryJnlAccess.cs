...
namespace Wiseman.PJC.Service.GroupSettings.RDB
{
    ...
    /// サンプルRDBアクセスクラス
    /// </summary>
    public class GroupCategoryJnlAccess : IGroupCategoryJnlAccess
    {
        /// <summary>Oracleアクセサ</summary>
        private IDBAccess _access;

        private readonly string _groupCategoryJnl_tb = "GRP_GROUPCATEGORYJNL";
        ...
        public GroupCategoryJnlAccess(IDBAccess access)
        {
            _access = access;
        }

        public int Create(GroupCategoryJnlEntity content)
        {
            var parameters = new Parameters();

            var sb = new StringBuilder();
            sb.AppendLine("INSERT INTO");
            sb.AppendLine($" {_groupCategoryJnl_tb}");
            sb.AppendLine("(");
            ...
            sb.AppendLine(", GROUPCATEGORY_CODE");
            sb.AppendLine(", GROUPCATEGORY_NAME");
            sb.AppendLine(", GROUPCATEGORY_KANA");
            sb.AppendLine(", GROUPCATEGORY_RYAKUSHO");
            sb.AppendLine(", GROUPTANI");
            ...
            sb.Append(SQLFunction.Char(content.GROUPCATEGORY_CODE));
            sb.Append(',');

            sb.Append(SQLFunction.Char(content.GROUPCATEGORY_NAME));
            sb.Append(',');

            sb.Append(SQLFunction.Char(content.GROUPCATEGORY_KANA));
            sb.Append(',');

            sb.Append(SQLFunction.Char(content.GROUPCATEGORY_RYAKUSHO));
            sb.Append(',');

            sb.Append(SQLFunction.Char(content.GROUPTANI));
            sb.Append(',');

            sb.Append(SQLFunction.Number(content.DISPLAY_ORDER));
            sb.Append(',');
